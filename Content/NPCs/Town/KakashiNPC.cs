...
namespace NarutoOverhaul.Content.NPCs.Town
{
	...
	// [AutoloadHead] registers KakashiNPC_Head.png (already present on disk) as this NPC's
	...
	public class KakashiNPC : ModNPC
	{
		...
		// sprite's head. Calibrated by pixel-measuring the bubble/head gap across two screenshots at
		// different offsets (24px: bubble still ~12px above head; 75px: bubble ~49px below/overlapping
		// the body) and solving the resulting linear fit for a ~0 gap - ~34px, not the 75px a rougher
		// "moved about the same distance past the head" eyeball estimate suggested.
